...
class TicTacToe:
    ...
    def button_clicked(self, i, j):
        ...
        if button['text'] == '':  # Only if the button hasn't been pressed
            button.config(text=self.turn, state='disabled')  # Mark button with 'X'
    ...
    def computer_move(self):
        ...
        if available_moves:
            i, j = random.choice(available_moves)  # Select random move
            self.buttons[i][j].config(text=self.turn, state='disabled')  # Mark button with 'O'
            self.check_for_winner()  # Check if game is over
            self.turn = 'X'  # Switch back to user's turn
    ...
    def reset(self):
        ...
        # If it's the computer's turn to start, make a move
        if self.turn == 'O':
            self.computer_move()
